...
node_t *evaluateList(context_t *env, node_t *node) {
  ...
  if (fn == NULL) {
    ...
    ast_node_list_t *retArgs = mkList();
    struct list_head *pos;
    list_for_each(pos, &(args->list)) {
      ast_node_list_t *entry = list_entry(pos, ast_node_list_t, list);
      append(evaluate(env, entry->car), retArgs);
    }
...
void addFunctionToContext(context_t *context, char *name, fn function) {
  if (context->size == context->max) {
    ...
    context->context =
        realloc(context->context, context->max * sizeof(context_obj_t *));
  }
