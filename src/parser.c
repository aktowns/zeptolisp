...
parser_result_t *parseList(const char *input) {
  ...
  ast_node_list_t *rootNode = NULL;
  int bfrOffset = 0;
  while ((currentNode = parse(bfr + bfrOffset))->consumed != 0) {
    bfrOffset += currentNode->consumed;
    if (rootNode == NULL) {
      rootNode = mkList(currentNode->value);
    } else if (currentNode->consumed != 0) {
      append(currentNode->value, rootNode);
    }
  }
  if (currentNode->consumed == 0)
    free(currentNode);
  free(bfr);
...
parser_result_t *parse(const char *input) {
  ...
  if (input == NULL || strlen(input) == 0) {
    ...
    pr->consumed = 0;
    pr->value = wrapNodeList(mkList(NULL));
    return pr;
  }
