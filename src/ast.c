...
ast_node_list_t *mkList(node_t* car) {
...
ast_node_list_t *cons(node_t *car, ast_node_list_t *cdr) {
  ast_node_list_t *cell = mkList(car);
  list_add_tail(&cell->list, &cdr->list);

  return cell;
}

ast_node_list_t *append(node_t *value, ast_node_list_t *init) {
  if (init == NULL)
    return mkList(NULL);

  ast_node_list_t *cell = mkList(value);
  list_add(&cell->list, &init->list);

  return cell;
}

ast_node_list_t *tail(ast_node_list_t *list) {
  return list_next_entry(list, list);
}
...
void listPP(ast_node_list_t *list) {
  ...
  list_for_each(pos, &(list->list)) {
    ast_node_list_t *entry = list_entry(pos, ast_node_list_t, list);
    _nodePP(entry->car);
  }
}
