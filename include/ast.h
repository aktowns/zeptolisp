...
typedef struct { char *value; } ast_node_string_t;

typedef struct { int64_t value; } ast_node_number_t;
...
typedef struct { char *value; } ast_node_symbol_t;

typedef struct { struct node *inner; } ast_node_quoted_t;
...
// static ast_node_list_t nil = { .car = NULL};
#define IS_NIL(x) (x != NULL && x->car == NULL)
